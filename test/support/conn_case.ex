defmodule WhirlWeb.ConnCase do
  @moduledoc """
  ...
  by setting `use WhirlWeb.ConnCase, async: true`, although
  ...
  def register_and_log_in_user(%{conn: conn} = context) do
    user = Whirl.AccountsFixtures.user_fixture()
    scope = Whirl.Accounts.Scope.for_user(user)
  ...
  def log_in_user(conn, user, opts \\ []) do
    token = Whirl.Accounts.generate_user_session_token(user)
  ...
  defp maybe_set_token_inserted_at(token, inserted_at) do
    Whirl.AccountsFixtures.override_token_inserted_at(token, inserted_at)
  end
end
